 ...
 * </copyright>
 */
package com.hankcs.hanlp.util;

import com.hankcs.hanlp.corpus.tag.PosTag;
import com.hankcs.hanlp.corpus.util.CustomPosTagUtil;
...
public class LexiconUtility
{
     ...
     * @return 包含词性与频次的信息
     */
    public static CoreDictionary.PosTagInfo getAttribute(String word)
    {
        CoreDictionary.PosTagInfo attribute = CoreDictionary.get(word);
     ...
     * @return 包含词性与频次的信息
     */
    public static CoreDictionary.PosTagInfo getAttribute(Term term)
    {
    ...
    {
        CoreDictionary.PosTagInfo attribute = getAttribute(word);
     ...
     * @return
     */
    public static boolean setAttribute(String word, CoreDictionary.PosTagInfo attribute)
    {
     ...
     * @return
     */
    public static boolean setAttribute(String word, PosTag... natures)
    {
        if (natures == null) return false;

        CoreDictionary.PosTagInfo attribute = new CoreDictionary.PosTagInfo(natures, new int[natures.length]);
    ...
    public static boolean setAttribute(String word, String... natures)
    {
        ...
        PosTag[] natureArray = new PosTag[natures.length];
        for (int i = 0; i < natureArray.length; i++)
        {
            natureArray[i] = PosTag.create(natures[i]);
        }
    ...
    {
        CoreDictionary.PosTagInfo attribute = CoreDictionary.PosTagInfo.create(natureWithFrequency);
        return setAttribute(word, attribute);
    }
     ...
     * @return 转换结果
     */
    public static PosTag convertStringToNature(String name, LinkedHashSet<PosTag> customNatureCollector)
    {
        try
        {
            return PosTag.valueOf(name);
        }
        catch (Exception e)
        {
            PosTag nature = CustomPosTagUtil.addPosTag(name);
