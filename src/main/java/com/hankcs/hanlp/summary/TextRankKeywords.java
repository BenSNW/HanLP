 ...
 * @author hankcs
 */
public class TextRankKeywords extends KeywordExtractor {
    ...
    {
        TextRankKeywords textRankKeyword = new TextRankKeywords();
     ...
     * @return
     */
    private Map<String,Float> getTermAndRank(String content, int size)
    {
        List<Term> termList = defaultSegment.seg(content);
        Map<String, Float> map = getRank(termList);
        MaxHeap<Map.Entry<String, Float>> maxHeap = new MaxHeap<>( size,
                Comparator.comparing(Map.Entry::getValue));
        maxHeap.addAll(map.entrySet());

        Map<String, Float> result = new LinkedHashMap<>();

        for (Map.Entry<String, Float> entry : maxHeap.toList())
        {
