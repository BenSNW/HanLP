 ...
 * @author hankcs
 */
public abstract class ShortestPathSegment extends Segment
{

    public ShortestPathSegment()
    {
        super();
    }
...
//            System.out.println("current:" + current + " next:" + next);
            PosTag currentNature = current.getNature();
            if (currentNature == PosTag.nx && (next.hasNature(PosTag.q) || next.hasNature(PosTag.n)))
            {
                ...
                if (param.length == 2)
                {
                    if (TextUtility.isAllNum(param[0]) && TextUtility.isAllNum(param[1]))
                    {
                        ...
                        current.confirmNature(PosTag.m);
        ...
        while (listIterator.hasNext())
        {
            ...
            if (TextUtility.isAllNum(current.realWord) || TextUtility.isAllChineseNum(current.realWord))
            {
                ...
                else
                {
                    ...
                    {
                        current.confirmNature(PosTag.t, true);
                    }
                    else
                    {
                        ...
                        {
                            current.confirmNature(PosTag.m, true);
                        }
                        //===== 5、当前串最后一个汉字是"∶·．／"和半角的'.''/'，且长度大于1，那么去掉最后一个字符。例如"1."
...
//        logger.trace("数字识别后：" + Graph.parseResult(linkedArray));
    ...
    protected void GenerateWordNet(final WordNet wordNetStorage)
    {
        ...
        // 核心词典查询
        DoubleArrayTrie<CoreDictionary.PosTagInfo>.Searcher searcher = CoreDictionary.trie.getSearcher(charArray, 0);
        while (searcher.next())
        {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            if (vertex.realWord.length() > 2)
            {
                ...
                while (currentLine < line + vertex.realWord.length())
                {
                    ...
                    {
                        if (
                                ((termMain.tag == PosTag.mq && smallVertex.hasNature(PosTag.q)) ||
