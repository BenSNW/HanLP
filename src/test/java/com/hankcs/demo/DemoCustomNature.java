...
public class DemoCustomNature
{
    ...
    {
        System.out.println("as".matches("^as将"));

        // 对于系统中已有的词性,可以直接获取
        PosTag pcNature = PosTag.fromString("n");
        System.out.println(pcNature);
        // 此时系统中没有"电脑品牌"这个词性
        pcNature = PosTag.fromString("电脑品牌");
        System.out.println(pcNature);
        // 我们可以动态添加一个
        pcNature = PosTag.create("电脑品牌");
        System.out.println(pcNature);
        // 可以将它赋予到某个词语
        ...
        System.out.println(termList);
        // 如果使用了动态词性之后任何类使用了switch(pos)语句,必须注册每个类:
        CustomPosTagUtil.registerSwitchClass(DemoCustomNature.class);
        for (Term term : termList)
        {
            switch (term.tag)
            {
